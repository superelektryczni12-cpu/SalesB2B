...
ipcMain.handle('team-data-load', () => backend.loadTeamData());
ipcMain.handle('team-data-save', (_e, userId, dataKey, value) => backend.saveTeamUserData(userId, dataKey, value));
ipcMain.handle('chat-threads-list', () => backend.listChatThreads());
ipcMain.handle('chat-messages-list', (_e, threadId) => backend.listChatMessages(threadId));
ipcMain.handle('chat-message-send', (_e, payload) => backend.sendChatMessage(payload));
ipcMain.handle('chat-file-upload', (_e, file) => backend.uploadChatFile(file));
ipcMain.handle('chat-file-url', (_e, filePath) => backend.createChatFileUrl(filePath));
ipcMain.handle('call-recording-upload', (_e, file) => backend.uploadCallRecording(file));
ipcMain.handle('call-recording-transcribe', (_e, recordingPath) => backend.transcribeCallRecording(recordingPath));
ipcMain.handle('brief-attachment-upload', (_e, file) => backend.uploadBriefAttachment(file));
ipcMain.handle('briefai-generate', (_e, payload) => backend.generateBriefAI(payload));
